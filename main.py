...
from data_gen import GeneratorSettings, generate_fc_network, get_submodels
...
def generate_submodels(args):
    ...
    for i in range(n_models):
        try:
            ...
            # rnd_model = generate_fc_network(settings)
            submodels = get_submodels(generate_fc_network(settings))
            for rnd_model in submodels:
                model_config = parse_keras_config(rnd_model, reuse_factor)
                # print(model_config)
        ...
        [
            {
                ...
                'json_path': f'./datasets/sequence/dataset-{proc}.json',
